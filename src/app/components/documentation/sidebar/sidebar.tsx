...
export default function Sidebar() {
    ...
    return (
        <aside className={`hidden flex-col flex-start flex-shrink-0 top-16 sticky h-[calc(100vh-4rem)] md:flex transition-all duration-300 ${collapsed ? 'w-1 ml-4' : 'w-64'}`}>
            ...
            <div className={`flex sticky bottom-0 bg-background py-4 border-t border-border gap-2 items-center mx-2 text-color-secondary ${collapsed ? 'flex-col' : 'flex-row'}`}>
                    ...
                    <Dropdown>
                        <DropdownTrigger onClick={toggleDropdown2}>
                            <Button variant="ghost" className="w-full !text-color-secondary" align="start">
                                ...
                                <span className={`${collapsed ? 'hidden' : 'visible'}`}>
                ...
                <Button variant="ghost" className="w-min" align="start" onClick={() => setCollapsed(!collapsed)}>
