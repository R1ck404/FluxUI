...
export default function Navbar() {
    ...
    return (
        <nav className="w-full min-h-20 h-20 flex items-center justify-between border-b border-border">
            <Button variant="ghost" className="fixed top-7 text-black dark:text-white md:hidden -translate-x-1/2 transform left-1/2" onClick={() => setIsOpen(!isOpen)}>
            ...
            <div className={`fixed top-20 left-0 w-full h-full bg-white dark:bg-zinc-900 z-50 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="flex flex-col items-center justify-center h-full space-y-4 text-black dark:text-white">
                    <h1 className="text-xl font-semibold mt-20 ">Getting Started</h1>
