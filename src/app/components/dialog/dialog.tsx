...
const Dialog = ({ open, onClose, className, children }: DialogProps) => {
        ...
        <>
            {visible && (
                <div className={classNames("fixed inset-0 z-50 overflow-y-auto flex justify-center items-center", className)}>
                    ...
                    ></div>
                    <div
                        className={`relative bg-background rounded-lg p-6 max-w-md w-full transition-all duration-300 ${animate ? "opacity-100 scale-100" : "opacity-0 scale-95"
                            }`}
                    >
...
const DialogTitle = ({ children, className }: { children: React.ReactNode; className?: string }) => {
    return <h2 className={classNames("text-lg font-semibold text-text", className)}>{children}</h2>;
};

const DialogDescription = ({ children, className }: { children: React.ReactNode; className?: string }) => {
    return <p className={classNames("text-sm text-secondary", className)}>{children}</p>;
};

const DialogContent = ({ children, className }: { children: React.ReactNode; className?: string }) => {
    return <>
        <hr className="my-4 border-border" />
        <p className={classNames("text-sm text-secondary", className)}>{children}</p>
    </>
};
